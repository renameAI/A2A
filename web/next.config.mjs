 ...
 *
 * **/saas/* 만 연다.** 이전에는 `/api/:path*` → `${ENGINE}/:path*` 캐치올이라
 * 인증이 없는 /product/*(22개)와 /v1/*(7개)가 공개 URL로 그대로 열렸다 —
 * 누구나 API 크레딧을 태우고 저장된 프로필을 덤프할 수 있었다(감사 확정).
 * 프록시는 신뢰 경계이므로, 여는 경로를 열거하는 쪽이 맞다.
 */
