...
### 5.1 상류 레버 R1~R4 — Represent·Retrieve (구현 완료)

Judge는 파이프라인 말단이다. 상류(Represent가 만드는 프로필, Retrieve의 랭킹)의 분산이 하류 전체에 전파되므로
같은 원리를 상류에 이식했다. 전부 **결정적 코드 게이트**(추가 LLM 호출 0):

**✅ R1 · stated 그라운딩 강등** → [`represent.py`](../app/engine/represent.py) `ground_profile`
`provenance=stated`인데 원문 3-gram 포함도 $g<0.15$(명백한 환각 신호)면 `inferred(conf 0.5)`로 **강등**(폐기 아님 —
라벨을 정직하게). bbox `grounding_score` 재사용. **L1과의 상호작용**: 강등된 필드(conf 0.5<0.6)는 질문 공리가
보강 질문을 **자동 재개방** — 환각이 신뢰 라벨을 달고 통과하는 대신 사람에게 확인 질문이 간다.
임계 0.15는 보수적이다: 추출값은 표준 한국어 패러프레이즈라 정당한 포함도가 낮을 수 있어, 근거가 거의 0인 것만 잡는다.

**✅ R2 · 근거 인용 실존 검증** → [`extractor.py`](../app/ingest/extractor.py)
`evidence_chunk_ids`가 실존 청크를 가리키는지 대조 — 환각 인용은 폐기+집계 로그. bbox의 "배치 밖 페이지 폐기"와 동일 계약.

**✅ R3 · 정규화 사영 (국가·산업)** → [`represent.py`](../app/engine/represent.py) `_canon_country/_canon_industry`
결정적 사영 $\Pi$ — 그 축의 표기 분산을 0으로. 실버그 예방이기도 하다: `industry_adjacent`가 문자열 일치 기반이라
"SaaS" vs "saas" 표기 요동이 **인접성 판정을 조용히 깨뜨렸다**(온톨로지 보정 +0.10 소실).

**✅ R4 · Retrieve 결정적 앵커 혼합 + 전순서** → [`retrieve.py`](../app/engine/retrieve.py)
(a) **앵커 혼합**: LLM 합성문(확률적 단일표본)이 전 후보 점수에 곱해지는 구조라 상류 한 표본이 랭킹 전체를 흔든다.
$\text{base} = \tfrac12\,\text{ov}(\text{synth},\cdot) + \tfrac12\,\text{ov}(\text{anchor},\cdot)$ (anchor=프로필 직접 도출 템플릿).
보너스 게이트는 **혼합 base $\ge 0.10$** 로 판정한다.
(b) **전순서 tie-break**: 동점 후보를 `(-score, company_id)`로 정렬 — 풀 순서에 따라 $k$ 경계에서 다른 후보가
잘리던 비재현성 제거. (c) 임계 경계 $|s-\tau|<0.03$ 후보 수를 정직 로그. (d) **앵커 강도 기권**: pool-max
$\text{ov}_\text{anchor}<0.05$ 면 과소정의 프로필로 저신뢰 경고(코인플립을 τ가 아닌 입력 품질 문제로 정직 표기).

**⟳ 실 LLM 캘리브레이션으로 재보정 (K-EXAONE 8회 × 2프로필, QC 4인 교차검증)** →
[`scripts/calibrate_retrieve.py`](../scripts/calibrate_retrieve.py). 실데이터가 이론 기반 수정을 교정한 대표 사례:
- **$\tau=0.12$ 유지** — 실측 청정갭 정중앙. well-defined 매칭 $\ge 0.176$ vs 노이즈 $\le 0.070$, 하향 τ교차 **0/96**.
  τ→0.15는 수치상 가능하나 아래 게이트 수정에 지배됨(노이즈 피팅). **F1(혼합이 강한 후보를 τ 아래로) 실측 반증.**
- **R4 순효과 = mixed**: well-defined는 **방어적**(분산 $0.27$~$0.50\times$ 감쇠 + run8 환각 synth에서 앵커가
  base를 게이트 위로 복원해 정당 후보 2건 구제), sparse는 무해한 희석.
- **RET-01 max 게이트는 과교정 — base 게이트로 되돌림.** 실측: max 게이트는 well-defined 통과 $24/24$로 이득 0인데
  sparse에서 $\text{ov}_s$ 스파이크 하나(anchor=0)로 위양성 **5**건 발생. base 게이트는 well-defined $24/24$ 보존
  (run8 구제 포함) + sparse 위양성 **5→0**. 신호 실재 판정은 '두 신호의 결합'이 옳다.
- **진짜 문제는 τ가 아니라 희소 프로필의 코인플립**(garbage-in) — 위 (d) 앵커 강도 기권으로 정직 처리.
