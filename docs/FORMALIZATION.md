...
스케일 구분(적대적 검토 F6 교정): **편차** 스케일 이론 감쇠 $0.5\times$, 실측 $0.32\times$ — **분산** 스케일로는 이론
$0.25\times$, 실측 $\approx 0.10\times$. 실측이 이론보다 강한 이유는 보너스 분기 뒤집힘 제거가 겹쳐서다(단, 커밋
c496159 시점 1회 측정 — 재현 스크립트 미보존을 명시함). 보너스 게이트는 혼합 base가 아니라
$\max(\text{ov}_s,\text{ov}_a)\ge 0.10$ 으로 판정(RET-01 절벽 수정).
(b) **전순서 tie-break**: 동점 후보를 `(-score, company_id)`로 정렬 — 풀 순서에 따라 $k$ 경계에서 다른 후보가
잘리던 비재현성 제거. (c) 임계 경계 $|s-\tau|<0.03$ 후보 수를 정직 로그(재실행 뒤집힘 위험 가시화).
⚠ **알려진 한계(F1, low)**: 혼합은 절대 점수 스케일을 낮추는데 $\tau=0.12$ 는 혼합 이전 스케일에서 보정된 값
그대로다 — 실데이터 실측(풀 전수 anchor overlap 0.031~0.325)에서 실피해는 반증됐지만, 실 LLM 합성으로 L0 하네스
재측정 후 $\tau$ 재보정이 원칙이다.
