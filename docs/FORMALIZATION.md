...
우선순위 순. 각 레버는 **계측 지표**를 동반해야 한다("못 재면 못 줄인다").

**L0 · 분산 계측 하네스 (선행 필수).**
동일 입력을 $m$ 회 재실행해 출력 분산을 재는 하네스. 지표: 범주형 필드 다수결 일치율, 스칼라 필드 표본표준편차,
핀 집합의 평균 Jaccard $\overline{|Y_i\cap Y_j|/|Y_i\cup Y_j|}$. **이게 없으면 아래 레버 효과가 전부 미검증.**

**L1 · `open_questions` 5공리를 코드로 집행.**
현재 5공리(원자성·판정가능성·비중복성·정보가치정렬·예산 $\le5$)는 `EXTRACT_SYSTEM` 프롬프트에만 있고 **집행기가 없다**(적대적 검증이
"미집행 제약집합 = 은유"로 판정). 코드 후처리로 강제: 질문 수 상한 컷, `stated`에서 도출가능 질문 제거, 대응 필드 없는 질문 폐기.
지표: 질문당 필드-확정률, 재실행 질문집합 Jaccard.

**L2 · 범주형 결정에 $k$-표본 다수결 + 일치율 보고.**
`judge`의 `decision` enum에 §4.3의 **지수적 집중** 다수결. 부산물로 **일치율(agreement) 자체가 보정 신호**(낮으면 그 케이스는
사람에게). 지표: mode 일치율, 결정 뒤집힘율 $2p(1-p)$ (임계 근처에서 최대).

**L3 · 소프트 LLM 게이트 → 하드 코드 게이트 이전.**
§2.3의 진단대로, 판단이 확률적 LLM 게이트에 있으면 분산이 붕괴 안 된다. 규칙으로 결정 가능한 컷(deal-breaker, 최소 프로필,
임계값)은 이미 코드 게이트다 — 남은 소프트 판단 중 규칙화 가능한 것을 이전.
