...
class OntologyAxis(BaseModel):
    ...
    value: str
    status: AxisStatus
    # 그 숫자를 그렇게 준 이유. 능력치 옆에 붙어 숫자를 읽을 수 있게 만든다 —
    # 근거 없는 숫자는 판정이 아니라 장식이다.
    why: str = ""
...
class CompanyOntology(BaseModel):
    ...
    # 문턱 — 요청 기업의 첫 콜드 아웃리치가 실무자 답장으로 이어질 확률(모델 판정).
    # 실측(귤메달): 1·2위가 롯데·현대백화점이었다 — 보완성·실존만 보면 대기업이
    # 이기지만, 제주 소규모 브랜드가 대기업 벤더 등록 절차를 첫 메일로 뚫을
    # 확률은 낮다. 닿을 수 없는 후보가 위에 있으면 목록 전체가 안 믿긴다.
