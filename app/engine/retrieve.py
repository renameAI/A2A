...
- 주제어만 넣으면 그 주제를 다룬 기사·블로그가 나온다. 그 일을 **하는 주체**를
  가리키는 명사를 반드시 넣어라. 그 명사는 이 건의 상대상에서 읽어낸다 —
  어떤 업종을 가정하지 말고, 상대상이 말하는 역할을 그 시장의 언어로 옮겨라.
- 기업 페이지를 뜻하는 말(회사소개·공식 사이트·기업정보·사업영역에 해당하는
  현지어 표현)을 함께 넣는다.
- 현지어를 섞는다 — 그 시장의 기업은 그 시장의 언어로 자신을 소개한다.
- 상대의 '문제'만 검색하면 그 문제를 다룬 기사가 나온다 — 문제를 겪는 **주체**를 검색한다.
- 업종이 바뀌면 주체 명사와 신호어가 통째로 바뀐다. 예시를 재사용하지 말고
  이 건의 상대상을 다시 읽어라.
...
def retrieve(req: RetrieveRequest,
    ...
    with progress.node("search", "하이브리드 검색 (2단)"):
        ...
        progress.log("검색", f"2단 — 하이브리드 검색 완료: {len(records)}건 중 "
                             f"임계 통과 {len(strong)}건 (경쟁사·무관 후보 강등)"
        ...
        if not strong:
            ...
            progress.log("검색", f"임계 통과 0건 — 상위 {len(strong)}건을 순위만으로 반환")
            ...
            if pad:
                strong = strong + pad
                progress.log("검색", f"임계 통과 {len(strong_ids)}건 < k={req.k} — 차순위 {len(pad)}건 보충")
...
- **체급을 본다.** 요청 기업의 규모·단계를 프로필에서 읽고, 각 경로에 대해
  그 체급의 회사가 첫 콜드 아웃리치로 실무자 답장을 받을 **가능성**을 reach로
  표시한다(high=닿기 쉬움 / mid / low=닿기 어려움). 대형 유통·대기업 조달은
  벤더 등록과 MD 절차 뒤에 있어 신생·소규모에게 low다 — 그런 경로만 내놓으면
  검색 결과 전체가 닿을 수 없는 목록이 된다(실측: 소규모 브랜드에게 백화점
  경로만 제안되어 상위가 전부 대기업이었다). **high 또는 mid 경로를 최소 2개**
  포함하라: 같은 체급의 전문 유통사·편집숍·지역 체인·온라인 셀렉트숍처럼
  의사결정이 짧은 상대가 거기 있다. low 경로를 빼라는 뜻이 아니다 — 성장
  방향으로 가치가 있으니 reach를 정직하게 붙여서 내놓아라.
  (후보 단위 reachability와 **같은 방향**이다: 클수록 닿기 쉽다. 방향이
  엇갈리면 화면에서 색과 뜻이 뒤집힌다.)
...
def propose_segments(req: RetrieveRequest) -> list[dict]:
    ...
    try:
        ...
        if out and all(s["reach"] == "low" for s in out if s["reach"]):
            from .. import progress
            progress.log("검색", "⚠ 제안된 경로가 전부 닿기 어려움 — 체급 맞는 "
