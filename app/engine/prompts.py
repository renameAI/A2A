...
각 층은 4~5문장 이내로 쓴다 (실측 사고: 자료가 사실·수치로 촘촘하면 층마다 인용을 다 \
욱여넣으려다 추론 토큰 예산을 끝까지 채우고 잘렸다 — 상한을 올려도 마찬가지였다, \
자료가 많을수록 그만큼 길어지는 구조라서다). 사실이 많을수록 **더 대표성 있는 것만 \
골라 압축**한다 — 전부 인용하지 않는다. 이건 정확도 손실이 아니라 편집이다: 다섯 겹 \
전부·인용 근거·추정 표시는 그대로 지키되, 층 하나에 사실 10개가 있으면 그중 판단에 \
가장 결정적인 2~3개만 쓴다.

⚠ 압축해도 2층 주어는 항상 고객이다. "우리가 처리한다"류는 1층·3층 몫, 2층에 쓰면 오답.
...
■ 추상화 규율 (핵심 필드) — 표면 명사가 아니라 결핍으로 쓴다. 교차 도메인 매칭이 \
여기 달려 있다 — 회사가 자신을 뭐라 부르는지가 아니라 어떤 결핍을 메우는지로.

**problem_solved와 solution은 절대 같은 문장이 되면 안 된다** (실측 사고: 둘이 거의 \
같아지자 검색 앵커가 "[내가 하는 일] 문제를 겪고 있어 [내가 하는 일] 같은 해법이 \
필요한"이라는 자기참조 문장이 되어, 풀 1,139개 중 651개가 임계를 넘고 무관한 회사가 \
상위에 올랐다). 두 필드는 **주어가 다르다**:
- problem_solved = **고객이 겪는 결핍·고통.** 주어는 고객이고, 우리 회사가 없어도 \
존재하는 문제다. 우리 제품·서비스 이름을 쓰지 마라.
- solution = **우리가 그 결핍을 메우는 방식.** 주어는 우리다.

| 회사 | ✗ problem_solved | ✓ problem_solved (고객의 결핍) | ✓ solution (우리의 방식) |
|---|---|---|---|
| 인테리어 | "노후 공간을 전환해준다" | "중소 호텔이 노후 객실 탓에 매출이 정체됐으나 리뉴얼 자본을 못 댄다" | "저자본·무철거로 경험형 상품 전환, 매출 쉐어로 회수" |
| AI 물류 | "예측 배차를 제공한다" | "중소 화주가 공차·반송으로 물류비를 새고 있다" | "수요 예측 기반 배차로 공차율을 낮춘다" |
| 핀테크 | "사기 탐지 API를 판다" | "온라인 가맹점이 결제 사기 차지백으로 손실을 본다" | "실시간 이상거래 탐지로 승인 전에 차단한다" |
| 에듀테크 | "원격 강사를 매칭한다" | "지방 중소 학원이 강사를 못 구해 폐강한다" | "검증된 원격 강사 풀로 수급을 메운다" |

자가 점검: problem_solved에서 우리 회사를 지워도 문장이 성립해야 한다. 성립하지 \
않으면 그건 solution을 쓴 것이다.
...
- Strategy Input(지역·타겟 유형)은 하드 필터가 아니라 씨앗으로 녹인다. 다만 \
**요청자가 실제로 닿을 수 있는 범위를 벗어나지 마라** — 지역이 지정됐으면 그 지역, \
없으면 요청자의 국가·산업 인접권 안에서 그린다. 근거 없는 타국·타대륙 주체를 \
지어내지 않는다.
...
- 출력은 검색어로 쓸 3~4문장의 한국어 서술문만. 설명·머리말 금지.
  (분량 상한을 두지 않는다 — 실측: 200~350자로 조이자 bigram 수가 줄어 overlap이
   이산적이 되고 랭킹 분산이 오히려 커졌다. top5 Jaccard 0.351→0.306, synth-only
   0.123→0.045. 짧은 검색어가 안정적일 것이라는 직관은 이 지표에선 틀렸다.)"""
