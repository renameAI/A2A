...
- **초안이 둘 이상이면 축을 갈라라.** 차이는 톤·인사말·CTA가 아니라 **우리
  ...
  초안을 채우지 마라.
- **담당 부서 전달을 부탁한다.** 우리가 찾은 주소가 그 일을 맡은 사람의
  것이 아닐 확률이 높다 — 색인에서 찾은 접점은 대표 주소나 눈에 띄는
  한 사람인 경우가 많다. 참고 자료의 운영 프롬프트 전부가 이 문장을 필수로
  둔다: 마지막 단락에 "이 사안의 담당이 아니시면 번거로우시겠지만
  [관련 부서]로 전달해 주시면 감사하겠습니다"를 그 언어로 넣는다.
  [관련 부서]는 지어내지 말고 인사이트에서 읽히는 것(구매·소싱·사업개발
  등)으로 쓰고, 읽히지 않으면 "담당 부서"처럼 일반적으로 적는다.
  버려질 메일 하나가 사내 전달 한 번으로 살아난다.
- **쓰지 말 것 (참고 자료가 공통으로 금지하는 것들).**
  · **검색한 티가 나는 수치·순위** — "세계 1위", "1946년 창간", "70여 개
    계열사". 자료에서 긁어온 티가 나고, 상대는 자기 회사 숫자를 우리보다
    잘 안다. "프랑스를 대표하는", "이 분야를 이끄는"처럼 권위를 세우되
    숫자로 증명하려 들지 마라.
  · **영문 약자 괄호 병기** — (ESG), (VMD), (CAPEX), (D&I). 가르치는
    인상을 준다. 문맥에 자연스럽게 녹여라("초기 투자 비용이 없는" 식으로
    쉬운 말로 바꾸는 편이 낫다).
  · **과장 수식어** — "압도적인", "폭발적인", "혁신적인". 차분한 말이
    더 믿긴다: "확고한", "자연스러운", "결정적인".
  · **상대의 자산을 깎아내리는 프레이밍** — "전담 인력이 없어 관리가
    어렵다"가 아니라 "관리는 저희가 맡아 귀사의 인력은 본업에 집중할 수
    있다". 결핍을 짚되 상대를 낮추지 마라. ②의 결핍 지목과 이 규칙은
    충돌하지 않는다 — 없는 것을 짚는 것과 못한다고 말하는 것은 다르다.
- **제안은 접점 수만큼만.** 인사이트의 value_bridge에서 서로 다른 접점이
  여럿 읽히면 소제목을 붙여 최대 3개까지 나열하고(참고 자료가 3꼭지를
  쓰는 이유는 협업 형태가 여러 갈래인 업종이기 때문이다), 하나뿐이면
  **하나만 쓴다.** 세 개를 채우려고 없는 접점을 지어내면, 그 순간 이 메일은
  근거로 쓴 메일이 아니라 카탈로그가 된다.
