...
- **무엇을 보고 연락하는지 밝힌다.** 킷에 근거 링크가 있으면 첫 단락에서
  ...
  넣지 마라.
- **길이와 구조.** 지금 나오는 초안은 서너 줄이라 상대가 "왜 우리인가"를
  알 수 없다. body는 **빈 줄로 나눈 네 단락**이고, 각 단락은 2~4문장이다.
  글자 수가 아니라 단락과 문장 수로 센다 — 실측: "400~700자"로만 적었더니
  스페인어 초안이 519자짜리 한 덩어리로 나왔다. 단락 사이에는 빈 줄
  (개행 두 번)을 반드시 넣어라. 네 단락은 이렇게 나눈다:
  ① 무엇을 보고 연락하는지(근거 링크와 함께) — 이 회사여야 하는 이유,
  ② 그래서 지금 무엇이 필요해 보이는지(observed_needs를 상대의 말로),
  ③ 우리가 대는 것과 그것이 맞닿는 지점(value_bridge) — 우리 소개는 여기
     한 단락으로 끝낸다. 회사 연혁·수상 이력 나열은 넣지 마라,
  ④ 부담 낮은 다음 행동 하나.
  각 단락은 2~4문장을 채운다. 한 문장짜리 단락이 네 개면 나눈 의미가 없다.
  길이를 채우려고 같은 말을 늘리지는 마라 — 근거가 두 개뿐이면 짧은 편이
  낫다. 늘려야 할 것은 문장이 아니라 상대가 확인할 수 있는 사실이다.
