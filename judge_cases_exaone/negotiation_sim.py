...
[K-EXAONE 판] 이 폴더 버전은 판단(EXTRACT·최종 판정·설명서·피드백 조율)은 물론
발화(SPEAK)·제안 메일·조정규칙 scope 심사까지 전부 K-EXAONE-236B(Friendli 전용
엔드포인트, OpenAI 호환)로 수행한다. 유일한 예외는 리서치(검색 그라운딩)로,
Google 검색이 Gemini 전용 기능이라 그 단계만 gemini-3.1-flash-lite 를 쓴다.
필요 패키지: pip install google-genai openai jsonschema
==================================================================
"""
...
# OpenAI 호환 SDK (K-EXAONE / Friendli 용).  pip install openai
from openai import OpenAI

# ==================================================================
# CONFIG  ── [K-EXAONE 판(版)] 판단은 K-EXAONE, 발화·리서치는 Gemini
# ==================================================================
...
try:
    from secrets_local import FRIENDLI_TOKEN, EXAONE_ENDPOINT_ID
except ImportError:
    FRIENDLI_TOKEN = _os.environ.get("FRIENDLI_TOKEN", "")
    EXAONE_ENDPOINT_ID = _os.environ.get("EXAONE_ENDPOINT_ID", "")

# 판단(EXTRACT·최종 판정·설명서·피드백 조율) = K-EXAONE-236B (Friendli 전용 엔드포인트)
# 발화(SPEAK)·메일·조정규칙 심사 = gemini-3.1-flash-lite / 리서치(검색 그라운딩) = Gemini 전용
FRIENDLI_BASE_URL = "https://api.friendli.ai/dedicated/v1"
EXAONE_LABEL = "K-EXAONE-236B (Friendli)"
ENABLE_THINKING = False   # Controllable Reasoning (False=빠름; True 로 올리면 판단 심층화)
PARSE_REASONING = True    # 본문에 최종답만 남김
CALL_TIMEOUT_S = 180      # 단일 API 호출 상한(초) — 초과 시 실패 처리 후 재시도(무한 대기 방지)
JUDGE_MODEL = EXAONE_LABEL          # 판단: chat()/chat_json() 이 이 라벨을 보면 K-EXAONE 으로 라우팅
SPEAK_MODEL = EXAONE_LABEL          # 발화·메일·scope 심사도 전부 K-EXAONE
RESEARCH_MODEL = "gemini-3.1-flash-lite"   # 검색 그라운딩 전용(Gemini 전용 기능) — 이 단계만 Gemini
MAX_ROUNDS = 7
...
PRICING = {"gemini-3.1-pro-preview": (2.00, 12.00), "gemini-3.1-flash-lite": (0.10, 0.40),
           # 전용 엔드포인트: 토큰당이 아니라 '가동시간' 과금 → 토큰량만 참고 표기
           "K-EXAONE-236B (Friendli)": (0.0, 0.0)}
PRICING_DEFAULT = (0.30, 2.50)
USD_TO_KRW = 1400
USAGE: Dict[str, Dict[str, Any]] = {}
PHASE_TIMES: Dict[str, Dict[str, Any]] = {}   # 단계명 -> {seconds, calls}
_CURRENT_PHASE = "기타"


def set_phase(name: str) -> None:
    """이후의 API 호출 시간을 이 단계명으로 집계한다."""
    global _CURRENT_PHASE
    _CURRENT_PHASE = name


def _record_usage(model: str, inp: int, out: int, secs: float = 0.0) -> None:
    u = USAGE.setdefault(model, {"input": 0, "output": 0, "calls": 0, "seconds": 0.0})
    u["input"] += inp; u["output"] += out; u["calls"] += 1
    u["seconds"] = u.get("seconds", 0.0) + secs
    p = PHASE_TIMES.setdefault(_CURRENT_PHASE, {"seconds": 0.0, "calls": 0})
    p["seconds"] += secs; p["calls"] += 1
...
def report_cost() -> None:
    ...
    for model, u in USAGE.items():
        ...
        total += c
        print(f"  {model}: 호출 {u['calls']}회 | 입력 {u['input']:,} · 출력 {u['output']:,} tok "
              f"| API 대기 {u.get('seconds', 0.0):,.1f}초 | ~${c:.4f}")
    print(f"  추정 총비용(Gemini 부분): ~${total:.4f} (약 {total*USD_TO_KRW:,.0f}원)")
    if EXAONE_LABEL in USAGE:
        print("  ※ K-EXAONE(Friendli 전용 엔드포인트)는 토큰당이 아니라 '엔드포인트 가동시간' "
              "기반 과금이라 위 금액에 미포함(토큰량만 참고 표기).")
    if PHASE_TIMES:
        print("\n=== [시간] 단계별 API 대기 시간 ===")
        tot = sum(p["seconds"] for p in PHASE_TIMES.values())
        for name, p in PHASE_TIMES.items():
            print(f"  {name}: {p['seconds']:,.1f}초 ({p['calls']}회 호출)")
        print(f"  합계: {tot:,.1f}초  ※ 단일 호출 상한 {CALL_TIMEOUT_S}초 초과 시 실패 처리 후 재시도")


# ==================================================================
# LLM 헬퍼 — Gemini(발화·리서치) + K-EXAONE(판단)
# ==================================================================
def make_client() -> genai.Client:
    if not GOOGLE_API_KEY:
        raise SystemExit("[설정 필요] GOOGLE_API_KEY 를 채우세요.")
    try:   # 단일 호출 타임아웃(무한 대기 방지) — 구버전 SDK 면 타임아웃 없이 생성
        return genai.Client(api_key=GOOGLE_API_KEY,
                            http_options=types.HttpOptions(timeout=CALL_TIMEOUT_S * 1000))
    except Exception:
        return genai.Client(api_key=GOOGLE_API_KEY)


_EXAONE_CLIENT: Optional[OpenAI] = None


def exaone_client() -> OpenAI:
    global _EXAONE_CLIENT
    if _EXAONE_CLIENT is None:
        if not FRIENDLI_TOKEN or not EXAONE_ENDPOINT_ID:
            raise SystemExit("[설정 필요] secrets_local.py 또는 환경변수에 "
                             "FRIENDLI_TOKEN / EXAONE_ENDPOINT_ID 를 채우세요.")
        # timeout: 단일 호출 상한(무한 대기 방지). 재시도는 우리 쪽 3회 루프가 담당.
        _EXAONE_CLIENT = OpenAI(api_key=FRIENDLI_TOKEN, base_url=FRIENDLI_BASE_URL,
                                timeout=CALL_TIMEOUT_S, max_retries=0)
    return _EXAONE_CLIENT


def chat_exaone(system: str, user: str, temperature: float = 0.4,
                max_tokens: int = 3000, json_mode: bool = False,
                top_p: float = 0.9, frequency_penalty: float = 0.0,
                presence_penalty: float = 0.0, thinking: Optional[bool] = None) -> str:
    """K-EXAONE(Friendli) 호출. json_mode 시 response_format=json_object 로
    JSON 문법을 디코딩 단계에서 강제(쓰레기 토큰 억제). top_p=0.9 로 저확률 토큰 차단.
    frequency/presence_penalty: 발화 반복성 억제용(발화 생성 호출에만 사용 — 판단 JSON 은 0 유지).
    thinking: 이 호출만 Controllable Reasoning 을 켜고/끄기(None=전역 ENABLE_THINKING)."""
    cli = exaone_client()
    ...
    for attempt in range(3):
        try:
            ...
            if text:
                return text
            last_err = RuntimeError("빈 응답")   # 빈 응답도 실패로 간주해 재시도
            # think 모드가 출력 토큰을 전부 소진해 본문이 비는 사례 → 재시도는 think 끔
            kwargs["extra_body"] = {**kwargs["extra_body"],
                                    "chat_template_kwargs": {"enable_thinking": False}}
            time.sleep(2 * (attempt + 1))
        except Exception as e:
            last_err = e
            # 페널티 파라미터가 문제일 수 있으니 재시도에서는 제거
            kwargs.pop("frequency_penalty", None)
            kwargs.pop("presence_penalty", None)
            # json_object 강제가 문제면 일반 모드로 낮춰 재시도(파싱은 extract_json 이 담당)
            if json_mode and "response_format" in kwargs:
                kwargs.pop("response_format", None)
                json_mode = False
            time.sleep(2 * (attempt + 1))
    raise RuntimeError(f"K-EXAONE 호출 실패: {last_err}")


def chat(client, model, system, user, temperature=0.4, max_tokens=3000,
         json_mode=False, use_search=False, frequency_penalty=0.0,
         presence_penalty=0.0, thinking=None) -> str:
    # K-EXAONE 라벨이면 Friendli 로 라우팅 — 검색 그라운딩만 Gemini 전용이라 예외
    if model == EXAONE_LABEL:
        if not use_search:
            return chat_exaone(system, user, temperature=temperature,
                               max_tokens=max_tokens, json_mode=json_mode,
                               frequency_penalty=frequency_penalty,
                               presence_penalty=presence_penalty, thinking=thinking)
        model = RESEARCH_MODEL   # 검색 호출이 EXAONE 라벨로 오면 안전하게 Gemini 로 전환
...
def clean_text(t: str) -> str:
    """EXAONE 저확률 깨진 토큰 방어: 한글(자모 포함)·ASCII·일반 문장부호 외 문자를 제거."""
    t = re.sub(r"[^가-힣ᄀ-ᇿ㄰-㆏\x20-\x7E\n·…‥ㆍ“”‘’—–%℃±×→]", "", t)
    return re.sub(r"  +", " ", t)


def chat_json(client, model, system, user, temperature=0.2, max_tokens=3000,
              frequency_penalty=0.0, presence_penalty=0.0, thinking=None) -> Dict[str, Any]:
    for _ in range(2):
        parsed = extract_json(chat(client, model, system, user,
                                   temperature=temperature, max_tokens=max_tokens, json_mode=True,
                                   frequency_penalty=frequency_penalty,
                                   presence_penalty=presence_penalty, thinking=thinking))
        # 객체 대신 배열이 오면 첫 dict 원소를 채택 (EXAONE 이 간혹 [{...}] 형태로 반환)
...
- [stance 일치] stance 는 발화의 실제 태도와 일치해야 한다. 제안·다음 단계·PoC 를
  제시하는 발화는 pursue 다. withdraw_* 는 발화 본문에 철수·중단 의사를 명시적으로
  밝힐 때만 사용하라.
- [반복 금지] 대화 기록에 있는 내 과거 발화와 같은 내용의 재진술 금지. 매 발화는
  (a) 새 정보 공개 (b) 양보·조건 수정 (c) 구체적 다음 단계 제안 중 최소 하나를 반드시
  포함하라. 상대가 같은 요구를 반복하면 같은 답을 되풀이하지 말고 관점을 바꿔라
  (지금 줄 수 없는 이유를 정직하게 공개, 대안 제시, 또는 한계 인정).
반드시 JSON 하나만 출력:
...
- [반복 금지] 같은 basis 를 두 번 이상 물을 때는 앞선 질문과 다르게 구성하라 —
  요구 항목 나열의 반복 금지, 우선순위 1개로 좁히거나 '이것만 확인되면 진행'식
  조건부 수용 기준을 제시. 같은 질문으로 더 진전이 없으면 남은 갭을 조건으로 남기고
  action="decide" 를 고려하라.
반드시 JSON 하나만 출력:
...
def research_company(client, name: str, hint: str) -> str:
    set_phase("리서치·프로필 생성")
    doc = chat(client, RESEARCH_MODEL,
...
def gen_emails(client, session: Dict[str, Any], hist_text: str) -> Dict[str, str]:
    ...
    b_mail = chat(client, SPEAK_MODEL, BUYER_EMAIL_SYS, ctx, temperature=0.5, max_tokens=2500,
                  frequency_penalty=0.3, presence_penalty=0.3)
    return {"seller": clean_text(s_mail).strip(), "buyer": clean_text(b_mail).strip()}
...
def _clean_strings(obj: Any) -> Any:
    """세션 트리 전체의 문자열에 clean_text 재귀 적용. 키에도 적용한다 —
    EXAONE 이 키 자체를 깨뜨린 실측 사례('ver dict')가 'verdict' 로 복원된다."""
    if isinstance(obj, str):
        return clean_text(obj)
    if isinstance(obj, list):
        return [_clean_strings(x) for x in obj]
    if isinstance(obj, dict):
        return {(clean_text(k) if isinstance(k, str) else k): _clean_strings(v)
                for k, v in obj.items()}
    return obj


def sanitize_session(session: Dict[str, Any]) -> None:
    # 0) 깨진 외국어 토큰 제거 — 판정 근거(rationale) 등 세션 내부 문자열 전체
    cleaned = _clean_strings(dict(session))
    session.clear()
    session.update(cleaned)
    states = {"bb": session["buyer_state"], "sb": session["seller_state"]}
    # status/verdict 열거형 정규화 — run() 내 보정과 이중화 (키 정리 후 재검증 경로 방어)
    for k, node in states["bb"].items():
        if isinstance(node, dict):
            if node.setdefault("status", "unknown") not in ("unknown", "assumed", "confirmed"):
                node["status"] = "assumed"
            if node.setdefault("verdict", "na") not in ("fit", "caution", "unfit", "na"):
                node["verdict"] = "na"
    for k, node in states["sb"].items():
        if isinstance(node, dict):
            if node.setdefault("status", "unknown") not in ("unknown", "assumed", "confirmed"):
                node["status"] = "assumed"
    ...
    # 판매자 outcome 열거형 보정 — 깨진 값(예: 'poc^POC_agreed')을 가장 가까운 정식 값으로
    _OUTCOMES = ["meeting_agreed", "poc_agreed", "deal_structured", "hold",
                 "walk_away_structural", "walk_away_values", "no_agreement"]
    so = session.get("seller_outcome", {})
    if isinstance(so, dict):
        ov = so.get("outcome")
        if isinstance(ov, str) and ov not in _OUTCOMES:
            low = ov.lower()
            fixed_ov = (next((o for o in _OUTCOMES if o in low), None)
                        or (difflib.get_close_matches(low, _OUTCOMES, n=1, cutoff=0.6) or ["hold"])[0])
            so["rationale"] = (str(so.get("rationale", "")) +
                               f" [정규화: outcome='{ov}'→'{fixed_ov}']").strip()
            so["outcome"] = fixed_ov
...
def run(scn_name: str, scn: Optional[Dict[str, str]] = None) -> None:
    ...
    stalemates = 0                      # 재시도로도 반복이 안 풀린 횟수 → 2회면 교착으로 조기 판정

    def _too_similar(new: str, prevs: List[str]) -> bool:
        return any(difflib.SequenceMatcher(None, new, p).ratio() >= REPEAT_THRESHOLD
                   for p in prevs)

    def antirepeat(res: Dict[str, Any], sys_prompt: str, user_msg: str,
                   prevs: List[str], counter: Optional[str] = None) -> tuple:
        """새 발화가 (a) 자기 과거 발화와 동일(반복) 또는 (b) 상대 직전 발화를 복창
        (에코)하면 1회 재시도(경고 지시 + 온도 상향 + think 켬).
        반환: (res, 'ok'|'retried'|'stalemate')."""
        utt = (res.get("utterance") or "").strip()
        if not utt:
            return res, "ok"
        self_rep = bool(prevs) and _too_similar(utt, prevs)
        echo = bool(counter) and _too_similar(utt, [counter])
        if not self_rep and not echo:
            return res, "ok"
        if echo and not self_rep:
            warn = ("\n\n[에코 경고] 방금 만든 발화가 '상대'의 직전 발화를 그대로 복창했다. "
                    "상대 발화를 인용·복창하지 말고, 내 입장에서 내 온톨로지 근거로 새 발화를 만들어라.\n"
                    f"[상대 직전 발화]\n{counter}")
        else:
            warn = ("\n\n[반복 경고] 방금 만든 발화가 아래 네 과거 발화와 사실상 동일했다. "
                    "같은 내용을 되풀이하지 말고 관점을 바꿔 다시 발화하라 — 새 정보 공개, "
                    "양보/조건 수정, 구체적 다음 단계 제안 중 하나를 반드시 포함할 것.\n"
                    + "\n".join(f"- {p}" for p in prevs[-3:]))
        try:   # 품질 개선용 재발화 — 실패해도 본 실행을 죽이지 않는다
            res2 = chat_json(client, SPEAK_MODEL, sys_prompt, user_msg + warn,
                             temperature=0.85, frequency_penalty=0.5,
                             presence_penalty=0.4, thinking=True)
        except Exception as e:
            print(f"  [재발화 호출 실패({e}) — 원 발화 유지]")
            return res, "stalemate"
        utt2 = (res2.get("utterance") or "").strip()
        if utt2 and not _too_similar(utt2, prevs or []) \
                and not (counter and _too_similar(utt2, [counter])):
            return res2, "retried"
        return (res2 if utt2 else res), "stalemate"

    def regen_if_empty(res: Dict[str, Any], sys_prompt: str, user_msg: str) -> Dict[str, Any]:
        """발화가 비었으면(깨진 토큰 제거 후 포함) think 를 켜고 1회 재생성."""
        if clean_text(res.get("utterance") or "").strip():
            return res
        try:   # 재생성 실패해도 본 실행을 죽이지 않는다
            res2 = chat_json(client, SPEAK_MODEL, sys_prompt,
                             user_msg + "\n\n[주의] utterance 필드는 절대 비워선 안 된다. "
                                        "반드시 한국어 발화를 채워라.",
                             temperature=0.7, thinking=True)
        except Exception:
            return res
        return res2 if clean_text(res2.get("utterance") or "").strip() else res

    # 인바운드: 구매자 첫 문의
    set_phase("협상 라운드(EXTRACT+SPEAK)")
    if scn["first_speaker"] == "buyer":
        bp = chat_json(client, SPEAK_MODEL, P_BS,
                       ...
                       temperature=0.6)
        b_utt = clean_text(bp.get("utterance") or "").strip() or "(발화 생성 실패)"
        print(f"\n[R0 구매자(인바운드)]\n  {b_utt}")
    ...
    for rnd in range(1, MAX_ROUNDS + 1):
        ...
        s_user = (f"[라운드 {rnd}]\n[대화 기록]\n{hist()}\n\n[현재 SB]\n{brief(sb, SB_IDS)}\n"
                  f"다음 발화를 만들어라.")
        sp = chat_json(client, SPEAK_MODEL, P_SS, s_user, temperature=0.6,
                       frequency_penalty=0.4, presence_penalty=0.3)
        sp = regen_if_empty(sp, P_SS, s_user)
        s_prevs = [t["text"] for t in transcript if t["who"] == "seller"]
        b_last = next((t["text"] for t in reversed(transcript) if t["who"] == "buyer"), None)
        sp, rep = antirepeat(sp, P_SS, s_user, s_prevs, counter=b_last)
        if rep == "retried":
            print(f"  [반복/에코 감지 → 재발화 성공]")
        elif rep == "stalemate":
            stalemates += 1
            print(f"  [교착 신호: 판매자 발화가 재시도 후에도 반복 ({stalemates}/2)]")
        s_utt = clean_text(sp.get("utterance") or "").strip() or "(발화 생성 실패)"
        stance = sp.get("stance", "pursue")
        if stance not in ("pursue", "hold", "withdraw_structural", "withdraw_values"):
            stance = "pursue"
        # stance 교차검증: 철수 라벨은 발화 본문이 실제 철수 선언일 때만 인정 (오라벨로
        # 협상이 1라운드에 잘못 끝나는 것 방지 — 확증 없으면 보수적으로 pursue)
        if stance in ("withdraw_structural", "withdraw_values"):
            try:
                chk = chat_json(client, JUDGE_MODEL,
                                "너는 발화 분류자다. 아래 발화가 '협상 철수·중단 선언'인지 판정한다. "
                                "제안·다음 단계·PoC 제시가 있으면 철수가 아니다. "
                                '반드시 JSON 하나만: {"withdraw": true|false}',
                                f"[발화]\n{s_utt}", temperature=0.0, max_tokens=200)
            except Exception:
                chk = {"withdraw": True}   # 확인 호출 실패 시 원 라벨 존중
            if chk.get("withdraw") is not True:
                print(f"  [stance 교차검증: 발화는 철수 선언이 아님 → '{stance}'→'pursue' 보정]")
                stance = "pursue"
        print(f"\n[R{rnd} 판매자 | move={sp.get('move','?')} | stance={stance}]\n  {s_utt}")
        transcript.append({"who": "seller", "text": s_utt, "round": rnd})
        sb["SB10_strategic_relationship"]["stance"] = stance
        if stance in ("withdraw_structural", "withdraw_values"):
            print("\n[판매자] 철수 — 협상 종료.")
            break
        ...
        b_user = (f"[라운드 {rnd}]\n[대화 기록]\n{hist()}\n\n[현재 BB]\n{brief(bb, BB_IDS)}\n\n"
                  f"[미확정 갭(우선순위순)]\n{bb_gaps(bb)}\n다음 발화 또는 결정을 만들어라.")
        bp = chat_json(client, SPEAK_MODEL, P_BS, b_user, temperature=0.6,
                       frequency_penalty=0.4, presence_penalty=0.3)
        bp = regen_if_empty(bp, P_BS, b_user)
        if bp.get("action") != "decide":   # 마무리 발화는 반복 검사 대상 아님
            b_prevs = [t["text"] for t in transcript if t["who"] == "buyer"]
            bp2, rep = antirepeat(bp, P_BS, b_user, b_prevs, counter=s_utt)
            if bp2.get("action") != "decide" or rep != "stalemate":
                bp = bp2
            if rep == "retried":
                print(f"  [반복/에코 감지 → 재발화 성공]")
            elif rep == "stalemate":
                stalemates += 1
                print(f"  [교착 신호: 구매자 발화가 재시도 후에도 반복 ({stalemates}/2)]")
        b_utt = clean_text(bp.get("utterance") or "").strip() or "(발화 생성 실패)"
        ...
        if action == "decide":
            ...
            break
        if stalemates >= 2:
            print("\n[교착 감지] 반복 발화 2회 — 라운드를 소진하지 않고 조기 최종 판정으로 이동.")
            break
    ...
    bf = chat_json(client, JUDGE_MODEL, P_BF, bf_user, max_tokens=4500)
    if not str(bf.get("rationale") or "").strip():   # 근거 공란이면 1회 재판정(비치명)
        print("  [최종 판정(구매자) rationale 공란 → 재판정]")
        try:
            bf = chat_json(client, JUDGE_MODEL, P_BF,
                           bf_user + "\n\n[주의] rationale 필드는 필수다. 결정 근거를 "
                                     "2문장 이상 반드시 서술하라.", max_tokens=4500) or bf
        except Exception:
            pass
    ...
    sf_user = (f"[대화 전체]\n{hist()}\n\n[최종 SB]\n{brief(sb, SB_IDS)}\n"
               f"[구매자 최종 결정] {buyer_decision['decision']}")
    sf = chat_json(client, JUDGE_MODEL, P_SF, sf_user, max_tokens=4500)
    if not str(sf.get("rationale") or "").strip():   # 근거 공란이면 1회 재판정(비치명)
        print("  [최종 판정(판매자) rationale 공란 → 재판정]")
        try:
            sf = chat_json(client, JUDGE_MODEL, P_SF,
                           sf_user + "\n\n[주의] rationale 필드는 필수다. outcome 근거를 "
                                     "2문장 이상 반드시 서술하라.", max_tokens=4500) or sf
        except Exception:
            pass
