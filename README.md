...
- [~] **Phase 6 — 운영화** (일부): 상태 영속화 완료 · 감사 로그(SYS-04) 기존 · Next.js 미착수
  - **상태 영속화**: `ProductStore`가 SQLite 백엔드([app/product/store.py](app/product/store.py)) —
...
| provider | judge 정확도 | retrieve top-1 | 안정성 |
|---|---|---|---|
| Mock (규칙 베이스라인) | 5/5 (1.00) | 1/1 (1.00) | 1.00 |
| K-EXAONE (retrieve) | — | 1/1 (top1 3/3) | 1.00 |

- anchor 케이스(divein×livi)가 **CoT #01 전문가 결론 `conditional`** 과 일치, deal-breaker(항공 대기업)는 결격 게이트로 `reject`, distractor(경쟁사·신축 럭셔리)는 `defer`.
- LLM judge 벤치마크는 케이스당 ~247s(deep reasoning)라 무료 API 통제로 보류 — 하네스는 `--only judge --k N`으로 준비됨.
- 정직성: Mock 100%는 골든셋이 시드 풀 설계에서 유래(회귀 가드·LLM 비교 베이스라인이 본 가치).

**엔진 안정성**: 자동화 테스트 **185건 통과**. 포스트닥급 적대적 검토(47 에이전트)에서 발견 13건 수정,
실 LLM 캘리브레이션으로 이론 기반 수정 1건(RET-01) 실측 교정.
